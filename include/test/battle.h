 ...
 * To build a ROM (pokefirered-test.elf) that can be opened in mgba to
 * view specific tests, e.g. Spikes ones, use:
 *     make pokefirered-test.elf TESTS='Spikes'
 *
 ...
 *   {
 *       GIVEN {
 *           ASSUME(gBattleMoves[MOVE_STUN_SPORE].flags.powderMove);
 ...
 * ASSUMEs which should apply to the whole file, e.g.:
 *     ASSUMPTIONS
 *     {
 ...
 *
 * KNOWN_FAILING
 * Marks a test as not passing due to a bug.
 * For example:
...
#define TEST_DEFAULT_MOVE MOVE_SPLASH
...
struct BattleTestData
{
    ...
    u8 explicitMoves[B_SIDE_COUNT];
    bool8 hasExplicitSpeeds;
    u8 explicitSpeeds[B_SIDE_COUNT];
    u16 slowerThan[B_SIDE_COUNT][PARTY_SIZE];
    u8 currentSide;
