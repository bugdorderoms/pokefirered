...
static void Task_DoLoadStartMenu(u32 taskId);
static void CreateStartMenuWindowAndList(void);
static void RemoveStartMenuWindow(void);
static u32 SetUpStartMenuItems(void);
static u32 DoLoadStartMenuItems(const struct StartMenuOption *options, u32 maxOptions);
static void StartMenu_MoveCursorFunc(s32 itemIndex, bool32 onInit, struct ListMenu *list);
static bool32 StartMenu_CanStartSelectedIndexAction(u32 index);
static void StartMenu_FadeScreenIfLeavingOverworld(u32 index);
static void DrawClockBox(void);
static void RemoveClockBox(void);
static u32 StartMenuDexnavCallback(void);
static u32 StartMenuPokedexCallback(void);
static u32 StartMenuPokemonCallback(void);
static u32 StartMenuBagCallback(void);
static u32 StartMenuPlayerCallback(void);
static u32 StartMenuPlayerLinkCallback(void);
static u32 StartMenuSaveCallback(void);
static u32 StartMenuOptionCallback(void);
...
static u32 SaveDialogCB_PrintAskOverwriteText(void);
static u32 SaveDialogCB_AskReplacePreviousFilePrintYesNoMenu(void);
static u32 SaveDialogCB_AskOverwriteOrReplacePreviousFileHandleInput(void);
static u32 SaveDialogCB_AskOverwritePrintYesNoMenu(void);
static u32 SaveDialogCB_PrintSavingDontTurnOffPower(void);
static u32 SaveDialogCB_DoSave(void);
static u32 SaveDialogCB_PrintSaveResult(void);
static u32 SaveDialogCB_WaitPrintSuccessAndPlaySE(void);
static u32 SaveDialogCB_WaitPrintErrorAndPlaySE(void);
static u32 SaveDialogCB_ReturnSuccess(void);
static u32 SaveDialogCB_ReturnError(void);
...
static bool32 DoDrawStartMenu(void)
{
...
static u32 CreateStartMenuWindow(u32 numItems)
{
...
static void CreateStartMenuWindowAndList(void)
{
    u32 numItems = SetUpStartMenuItems(), maxShowed = numItems > 9 ? 9 : numItems;
...
static u32 StartMenu_HandleInput(void)
{
...
void Task_StartMenuHandleInput(u32 taskId)
{
    ...
    case INPUT_STATE_RUN_OPTION_CALLBACK:
        switch (sStartMenuOptions[sStartMenu.selectedIndex].func.u32_void())
        {
...
static void SaveStatToString(u32 caseId, u8 *dest, u32 color)
{
...
static void PrintSaveTextWithFollowupFunc(const u8 *str, u32 (*saveDialogCB)(void))
{
...
static bool32 SaveDialog_Wait60FramesOrAButtonHeld(void)
{
...
static bool32 SaveDialog_Wait60FramesThenCheckAButtonHeld(void)
{
...
static void Task_SaveGame(u32 taskId)
{
    switch (RunSaveDialogCB())
    {
    case SAVECB_RETURN_CONTINUE:
...
static bool32 DoSetUpSaveAfterLinkBattle(u8 *state)
{
    switch (*state)
    {
    case 0:
...
static void Task_SaveGameAfterLinkBattle(u32 taskId)
{
    s16 *data = gTasks[taskId].data;
