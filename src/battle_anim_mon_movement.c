...
static void AnimTask_ShakeMonStep(u32 taskId);
static void AnimTask_ShakeMon2Step(u32 taskId);
static void AnimTask_ShakeMonInPlaceStep(u32 taskId);
static void AnimTask_ShakeAndSinkMonStep(u32 taskId);
static void AnimTask_ShakeTargetBasedOnMovePowerOrDmgStep(u32 taskId);
static void AnimTask_TranslateMonEllipticalStep(u32 taskId);
...
static void AnimTask_WindUpLungePart1(u32 taskId);
static void AnimTask_WindUpLungePart2(u32 taskId);
static void AnimTask_SlideOffScreen_Step(u32 taskId);
static void AnimTask_SwayMonStep(u32 taskId);
static void AnimTask_ScaleMonAndRestoreStep(u32 taskId);
static void AnimTask_RotateMonSpriteToSide_Step(u32 taskId);
...
{
    u32 i, wavePeriod = 1, spriteId = GetAnimBattlerSpriteId(gBattleAnimArgs[0]);;
...
static void AnimTask_SwayMonStep(u32 taskId)
{
    u32 spriteId = gTasks[taskId].data[4];
    u32 sineIndex = gTasks[taskId].data[10] + gTasks[taskId].data[2];
