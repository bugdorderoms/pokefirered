...
const struct BattleWeatherInfo gBattleWeatherInfo[ENUM_WEATHER_COUNT] =
{
    [ENUM_WEATHER_RAIN] =
    {
...
static u32 CanBeStatused(u32 attacker, u32 defender, u32 flags)
{
    ...
    switch (GetBattlerAbility(defender))
    {
        case ABILITY_LEAF_GUARD:
            if (GetBattlerWeatherFlags(defender) & B_WEATHER_SUN_ANY)
...
u32 CanBePutToSleep(u32 attacker, u32 defender, u32 flags)
{
    ...
    if ((ret = CanBeStatused(attacker, defender, flags)) != STATUS_CHANGE_WORKED)
        return ret;
...
static bool32 CanPoisonType(u32 attacker, u32 defender)
{
    if (GetBattlerAbility(attacker) == ABILITY_CORROSION && GetBattleMoveSplit(gCurrentMove) == SPLIT_STATUS)
        return TRUE;
...
bool32 CanBeFrozen(u32 attacker, u32 defender, u32 flags)
{
    ...
    if (IsBattlerOfType(defender, TYPE_ICE)) // Check types
        return FALSE;
    else if (GetBattlerWeatherFlags(defender) & B_WEATHER_SUN_ANY) // Check weather
        return FALSE;
...
void MarkBattlerForControllerExec(u32 battlerId)
{
    if (gBattleTypeFlags & BATTLE_TYPE_LINK)
        gBattleControllerExecFlags |= Bit(battlerId) << 0x1C;
    else
...
void MarkBattlerReceivedLinkData(u32 battlerId)
{
    ...
    for (i = 0; i < count; ++i)
        gBattleControllerExecFlags |= Bit(battlerId) << (i << 2);
...
static u32 GetBattlerSupressedAbility(u32 battler)
{
    if (gStatuses3[battler] & (STATUS3_GASTRO_ACID | STATUS3_ABILITY_NULLIFIED)) // Check battle effects of suppression
        return ABILITY_NONE;
...
static bool32 IsNeutralizingGasOnField(u32 ability)
{
    u32 i;
    
    if (!gAbilities[ability].cantBeSuppressed)
    {
        for (i = 0; i < gBattlersCount; i++)
        {
...
static inline bool32 IsMoldBreakerTypeAbility(u32 battlerId)
{
    u32 attackerAbility = GetBattlerAbility(battlerId);
    
    if (attackerAbility == ABILITY_MOLD_BREAKER || attackerAbility == ABILITY_TERAVOLT || attackerAbility == ABILITY_TURBOBLAZE
    || (attackerAbility == ABILITY_MYCELIUM_MIGHT && GetBattleMoveSplit(gCurrentMove) == SPLIT_STATUS) || gBattleMoves[gCurrentMove].flags.ignoreAbilities)
        return TRUE;
...
{
    u32 ability = GetBattlerSupressedAbility(battler);
    
    // Can't suppress self
    if (ability != ABILITY_NEUTRALIZING_GAS && IsNeutralizingGasOnField(ability))
        return ABILITY_NONE;
    
    if (gAbilities[ability].breakable && battler != gCurrentTurnActionBattlerId && !gBattleStruct->bypassMoldBreakerChecks)
    {
        if ((BattleAI_IsRunning() || (gCurrentTurnActionNumber < gBattlersCount && gBattlerByTurnOrder[gCurrentTurnActionNumber] == gCurrentTurnActionBattlerId
...
static void TryEndAbilityEffectOnSuppression(u32 battlerId, u32 oldAbility, u32 newAbility)
{
...
{
    u32 oldAbility = gBattleMons[battlerId].ability;
...
{
    u32 suppressedAbility = gBattleMons[battlerId].ability;
...
void ResetSentPokesToOpponentValue(void)
{
    ...
    for (i = 0; i < gBattlersCount; i += 2)
        bits |= Bit(gBattlerPartyIndexes[i]);
...
void UpdateSentPokesToOpponentValue(u32 battler)
{
    ...
    else
    {
        ...
        for (i = 1; i < gBattlersCount; ++i)
            gSentPokesToOpponent[(i & BIT_FLANK) >> 1] |= Bit(gBattlerPartyIndexes[battler]);
    }
}
...
static u32 GetImprisonedMovesCount(u32 battlerId, u32 move)
{
    u32 i, imprisonedMoves = 0;
...
bool32 TrySetCantSelectMoveBattleScript(u32 battlerId, u32 movePos)
{
    ...
    u32 holdEffect = GetBattlerItemHoldEffect(battlerId, TRUE);
    bool32 isDynamaxed = IsGimmickActiveOrSelected(battlerId, GIMMICK_DYNAMAX);
    ...
    if (!isDynamaxed)
    {
        ...
        else if (gBattleStruct->battlers[battlerId].choicedMove && gBattleStruct->battlers[battlerId].choicedMove != MOVE_UNAVAILABLE
        && gBattleStruct->battlers[battlerId].choicedMove != move)
        {
    ...
    if (gDisableStructs[battlerId].tauntTimer && GetBattleMoveSplit(move) == SPLIT_STATUS)
    {
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_MOVE_TAUNTED;
        cantSelect = TRUE;
    }
    else if (!gBattleMons[battlerId].pp[movePos])
    {
    ...
    if (cantSelect)
    {
        if (!isDynamaxed)
            gCurrentMove = move;
        else // Display Max Move's name in the strings
            gCurrentMove = GetMaxMove(battlerId, gBattleMons[battlerId].species, move, gBattleMoves[move].type, gBattleMoves[move].split);
...
{
    u32 i, move, unusableMoves = 0, holdEffect = GetBattlerItemHoldEffect(battlerId, TRUE);
    bool32 isDynamaxed = IsGimmickActiveOrSelected(battlerId, GIMMICK_DYNAMAX);
...
{
    u32 unusable = CheckMoveLimitations(battlerId, 0);
...
static bool32 HadMoreThanHalfHpNowDoesnt(u32 battlerId)
{
...
static bool32 IsImmuneToWeatherDamage(u32 battlerId, u32 ability)
{
    ...
    || !(GetBattlerWeatherFlags(battlerId) & B_WEATHER_ANY) || ability == ABILITY_OVERCOAT || ability == ABILITY_MAGIC_GUARD)
        return TRUE;
    return FALSE;
}
...
enum
{
    ...
    ENDTURN_TAUNT_ENDS, 
    ENDTURN_TORMENT_ENDS, // From G-Max Meltdown
    ENDTURN_ENCORE_ENDS,
...
bool32 DoEndTurnEffects(void)
{
    ...
    do
    {
        ...
        switch (gBattleStruct->turnEffectsTracker)
        {
            ...
            {
                u32 weatherEnumId = GetCurrentWeatherEnumId(gBattleWeather);
                ...
                if (weatherEnumId != ENUM_WEATHER_COUNT)
                {
                    if (gBattleStruct->weatherDuration > 0 && --gBattleStruct->weatherDuration == 0)
                    {
                        ...
                        gBattleCommunication[MULTISTRING_CHOOSER] = gBattleWeatherInfo[weatherEnumId].endMessage;
                        BattleScriptExecute(BattleScript_WeatherEnds);
                    }
                    else
                    {
                        gBattleScripting.animArg1 = gBattleWeatherInfo[weatherEnumId].animation;
                        gBattleCommunication[MULTISTRING_CHOOSER] = gBattleWeatherInfo[weatherEnumId].continueMessage;
            ...
            case ENDTURN_WEATHER_DAMAGE:
                if (IsBattlerAlive(gBattlerAttacker))
                {
                    ...
                    {
                        u32 weatherFlags = GetBattlerWeatherFlags(gBattlerAttacker);
                        
                        if (weatherFlags & B_WEATHER_SANDSTORM)
                        {
                            if (!IsBattlerAnyType(gBattlerAttacker, TYPE_ROCK, TYPE_STEEL, TYPE_GROUND) && ability != ABILITY_SAND_VEIL && ability != ABILITY_SAND_RUSH
                        ...
                        {
                            if (!IsBattlerOfType(gBattlerAttacker, TYPE_ICE) && ability != ABILITY_SNOW_CLOAK && ability != ABILITY_ICE_BODY)
                            {
            ...
            case ENDTURN_LEECH_SEED_DRAIN:
                if (IsBattlerAlive(gBattlerTarget) && (gStatuses3[gBattlerTarget] & STATUS3_LEECHSEED) && IsBattlerAlive(gDisableStructs[gBattlerTarget].leechSeedBattler)
            ...
            case ENDTURN_BURN_DAMAGE:
                if (IsBattlerAlive(gBattlerAttacker) && gBattleMons[gBattlerAttacker].status1.id == STATUS1_BURN && ability != ABILITY_MAGIC_GUARD)
                {
#if BURN_UPDATE
                    gBattleMoveDamage = GetNonDynamaxMaxHP(gBattlerAttacker) / 16;
#else
            ...
            case ENDTURN_NIGHTMARE_DAMAGE:
                ...
                && (gBattleMons[gBattlerAttacker].status1.id == STATUS1_SLEEP || ability == ABILITY_COMATOSE) && ability != ABILITY_MAGIC_GUARD)
                {
            ...
            case ENDTURN_BINDING_COUNTER:
                if (IsBattlerAlive(gBattlerAttacker) && gDisableStructs[gBattlerAttacker].wrapTurns)
                {
                    ...
                    {
                        // It doesn't prevent the binding counter from decreasing
                        if (ability != ABILITY_MAGIC_GUARD)
                        {
                            ...
                            gBattleScripting.animArg1 = gBattleMoves[gDisableStructs[gBattlerAttacker].wrappedMove].argument.generic;
            ...
            case ENDTURN_TAUNT_ENDS:
                ...
                IncrementBattlerBasedEndTurnEffects();
                break;
            case ENDTURN_TORMENT_ENDS:
                if ((gBattleMons[gBattlerAttacker].status2 & STATUS2_TORMENT) && gDisableStructs[gBattlerAttacker].tormentTimer > 0
                && --gDisableStructs[gBattlerAttacker].tormentTimer == 0)
            ...
            case ENDTURN_AURORA_VEIL_ENDS:
                if ((gSideStatuses[side] & SIDE_STATUS_AURORA_VEIL) && gSideTimers[side].auroraVeilTimer > 0 && --gSideTimers[side].auroraVeilTimer == 0)
                {
            ...
            case ENDTURN_EJECT_PACK:
                IncrementBattlerBasedEndTurnEffects();
                break;
            case ENDTURN_DYNAMAX_ENDS:
                if (IsBattlerAlive(gBattlerAttacker) && GetActiveGimmick(gBattlerAttacker) == GIMMICK_DYNAMAX && !IsRaidBoss(gBattlerAttacker)
                && gBattleStruct->battlers[gBattlerAttacker].dynamaxTurns > 0 && --gBattleStruct->battlers[gBattlerAttacker].dynamaxTurns == 0)
                {
                    if (TryRevertGigantamax(gBattlerAttacker))
                        BattleScriptExecute(BattleScript_GigantamaxEnds);
                    else
    ...
    do
    {
        switch (gBattleStruct->atkCancellerTracker)
        {
            ...
            case CANCELLER_RAID_BATTLE_NULLIFICATION:
                if (IsRaidBoss(gBattlerAttacker) && RandomPercentage(RNG_RAID_STATS_NULLIFICATION, GetRaidBattleStatNullificationChance(gBattlerAttacker)))
                {
                    u32 i;
                    
                    // Reset raid boss stat stages
                    for (i = 0; i < NUM_BATTLE_STATS; i++)
                    {
                        if (gBattleMons[gBattlerAttacker].statStages[i] < DEFAULT_STAT_STAGES)
                            gBattleMons[gBattlerAttacker].statStages[i] = DEFAULT_STAT_STAGES;
                    }
                    BattleScriptCall(BattleScript_RaidBattleStatNullification);
                    ++effect;
                }
                ++gBattleStruct->atkCancellerTracker;
                break;
            ...
            case CANCELLER_FROZEN: // check being frozen
                ...
                if (gBattleMons[gBattlerAttacker].status1.id == STATUS1_FREEZE && !gBattleMoves[gCurrentMove].flags.thawUser)
                {
                    ...
                    {
                        ClearBattlerStatus(gBattlerAttacker);
                        PrepareBattlerNickWithPrefixBuffer(gBattleTextBuff1, gBattlerAttacker);
            ...
            case CANCELLER_GRAVITY_2:
                if ((gFieldStatus & STATUS_FIELD_GRAVITY) && gBattleMoves[gCurrentMove].flags.gravityBanned && !IsMaxMove(gCurrentMove))
                {
            ...
            case CANCELLER_DISABLED: // disabled move
                if (gDisableStructs[gBattlerAttacker].disabledMove && gDisableStructs[gBattlerAttacker].disabledMove == gCurrentMove && !IsMaxMove(gCurrentMove))
                {
            ...
            case CANCELLER_HEAL_BLOCKED_2:
                if ((gStatuses3[gBattlerAttacker] & STATUS3_HEAL_BLOCK) && GET_MOVE_MOVEEFFECT_TABLE(gCurrentMove).healingEffect && !IsMaxMove(gCurrentMove))
                {
            ...
            case CANCELLER_TAUNTED: // taunt
                if (gDisableStructs[gBattlerAttacker].tauntTimer && GetBattleMoveSplit(gCurrentMove) == SPLIT_STATUS && !IsMaxMove(gCurrentMove))
                {
            ...
            case CANCELLER_IMPRISONED: // imprisoned
                if (!IsMaxMove(gCurrentMove) && GetImprisonedMovesCount(gBattlerAttacker, gCurrentMove))
                {
            ...
            case CANCELLER_PARALYSED: // paralysis
                if (gBattleMons[gBattlerAttacker].status1.id == STATUS1_PARALYSIS && !RandomPercentage(RNG_PARALYSIS, 75))
                {
            ...
            case CANCELLER_STANCE_CHANGE: // check Stance Change
                ...
                {
                    u32 newSpecies = TryDoBattleFormChange(gBattlerAttacker, FORM_CHANGE_MOVE);
                ...
                if (gBattleMons[gBattlerAttacker].status2 & STATUS2_BIDE)
                {
                    ...
                    else
                    {
                        if (gBattleStruct->battlers[gBattlerAttacker].bideTakenDamage)
                        {
                            ...
                            if (!IsBattlerAlive(gBattlerTarget))
                                gBattlerTarget = gBattleStruct->battlers[gBattlerAttacker].moveTarget = GetMoveTarget(gCurrentMove, MOVE_TARGET_SELECTED + 1, FALSE);
            ...
            case CANCELLER_THAW: // move thawing
                ...
                ++gBattleStruct->atkCancellerTracker;
                break;
            case CANCELLER_DYNAMAX_MOVES:
                if ((GetActiveGimmick(gBattlerTarget) == GIMMICK_DYNAMAX && IsMoveBlockedByDynamax(gCurrentMove)) || IsMoveBlockedByRaidBattle(gBattlerTarget, gCurrentMove))
                {
            ...
            case CANCELLER_PRIMAL_WEATHER: // primal weather
                if (GetBattleMoveSplit(gCurrentMove) != SPLIT_STATUS)
                {
                    ...
                    {
                        case TYPE_FIRE:
                            if (GetBattlerWeatherFlags(gBattlerAttacker) & B_WEATHER_RAIN_PRIMAL)
                            {
                            ...
                            break;
                        case TYPE_WATER:
                            if (GetBattlerWeatherFlags(gBattlerAttacker) & B_WEATHER_SUN_PRIMAL)
                            {
            ...
            case CANCELLER_PRANKSTER: // check Prankster
                if (GetBattlerAbility(gBattlerAttacker) == ABILITY_PRANKSTER && GetBattleMoveSplit(gCurrentMove) == SPLIT_STATUS && IsBattlerAlive(gBattlerTarget)
            ...
            case CANCELLER_DAMP:
                ...
                {
                    u32 battler = ABILITY_ON_FIELD(ABILITY_DAMP);
...
static bool32 GetBattlerPartyRange(u32 battlerId, u32 *firstMonId, u32 *lastMonId)
{
    bool32 isMulti = TRUE;
    
    if (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER)
    {
        battlerId = ((battlerId & BIT_FLANK) / 2);
        
        *firstMonId = battlerId * MULTI_PARTY_SIZE;
        *lastMonId = *firstMonId + MULTI_PARTY_SIZE;
    }
    else if (!(gBattleTypeFlags & BATTLE_TYPE_MULTI) || ((gBattleTypeFlags & BATTLE_TYPE_RAID) && GetBattlerSide(battlerId) == B_SIDE_OPPONENT))
    {
        *firstMonId = 0, *lastMonId = PARTY_SIZE;
        isMulti = FALSE;
    }
    else
    {
        battlerId = GetLinkTrainerFlankId(GetBattlerMultiplayerId(battlerId));
...
bool32 HasNoMonsToSwitch(u32 battler, u32 partyIdBattlerOn1, u32 partyIdBattlerOn2)
{
    ...
    {
        u32 i, playerId, flankId, firstMonId, lastMonId;
        struct Pokemon *party = GetBattlerParty(battler);

        if (GetBattlerPartyRange(battler, &firstMonId, &lastMonId))
        {
...
static bool32 TryActivateCommander(u32 battlerId, bool32 cancelAction)
{
...
{
    bool32 affectsUser, blocked = FALSE;
    u32 i, moveTarget = GetBattlerMoveTargetType(attacker, move);
    
    if (moveTarget != MOVE_TARGET_OPPONENTS_FIELD)
    {
        affectsUser = (attacker == target);
        
        // Check target's ability
        if (IsBattlerAlive(target))
        {
            switch (GetBattlerAbility(target))
            {
                case ABILITY_SOUNDPROOF:
                    if (gBattleMoves[move].flags.soundMove && !affectsUser)
                    {
                ...
                case ABILITY_BULLETPROOF:
                    if (gBattleMoves[move].flags.ballisticMove && !affectsUser)
                    {
                        gBattleScripting.battler = target;
                        blocked = TRUE;
                    }
                ...
                case ABILITY_TELEPATHY:
                    if (IsBattlerAlive(BATTLE_PARTNER(target)) && attacker == BATTLE_PARTNER(target) && GetBattleMoveSplit(move) != SPLIT_STATUS)
                    {
                        gBattleScripting.battler = target;
                        blocked = TRUE;
                    }
                ...
                case ABILITY_GOOD_AS_GOLD:
                    if (GetBattleMoveSplit(move) == SPLIT_STATUS && !affectsUser)
                    {
                        gBattleScripting.battler = target;
                        blocked = TRUE;
                    }
        ...
        // Check abilities that protects the whole side
        for (i = target; i < NUM_BATTLERS_PER_SIDE && !blocked; i = BATTLE_PARTNER(i))
        {
            if (IsBattlerAlive(i))
            {
                switch (GetBattlerAbility(i))
                {
                    ...
                    case ABILITY_ARMOR_TAIL:
                        if (GetMovePriority(attacker, move) > 0 && !IsBattlerAlly(attacker, i) && !IsZMove(move) && (moveTarget != MOVE_TARGET_ALL_BATTLERS
                        || gCurrentMove == MOVE_PERISH_SONG || gCurrentMove == MOVE_FLOWER_SHIELD || gCurrentMove == MOVE_ROTOTILLER))
                        {
        ...
        if (blocked && !onlyChecking)
        {
            if (gBattleMons[attacker].status2 & STATUS2_MULTIPLETURNS)
...
u32 CanAbilityAbsorbMove(u32 ability, u32 move, u32 moveType, u32 attacker, u32 target, bool32 onlyChecking)
{
    ...
    if (attacker != target && GetBattlerMoveTargetType(attacker, move) != MOVE_TARGET_OPPONENTS_FIELD)
    {
        statAmount = +1;
        ...
        switch (ability)
        {
            ...
            case ABILITY_WIND_RIDER:
                if (gBattleMoves[move].flags.windMove)
                {
                    statId = STAT_ATK;
                    effect = 2;
                }
        ...
        if (!onlyChecking)
        {
            ...
            {
                case 1:
                    if (gProtectStructs[attacker].notFirstStrike)
                        gBattlescriptCurrInstr = BattleScript_MoveHPDrain;
                    else
                        gBattlescriptCurrInstr = BattleScript_MoveHPDrain_PPLoss;
                    
                    if (BATTLER_MAX_HP(target) || (gStatuses3[target] & STATUS3_HEAL_BLOCK))
                    {
                        if (gProtectStructs[attacker].notFirstStrike)
                    ...
                    {
                        gBattleMoveDamage = GetNonDynamaxMaxHP(target) / 4;
                        if (gBattleMoveDamage == 0)
                            gBattleMoveDamage = 1;
                        gBattleMoveDamage *= -1;
                    }
                    break;
                case 2:
                    if (CompareStat(target, statId, MAX_STAT_STAGES, CMP_NOT_EQUAL))
                    {
                ...
                case 3:
                    ...
                    if (!gDisableStructs[target].flashFireBoost)
                    {
                        gDisableStructs[target].flashFireBoost = TRUE;
...
{
    u32 i, moveType, ability, effect = 0;
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_SAFARI))
    {
        ...
        switch (caseId)
        {
            case ABILITYEFFECT_ON_SWITCHIN:
                if (!(IS_BATTLE_TYPE_GHOST_WITHOUT_SCOPE && gAbilities[ability].cantAffectGhost))
                {
                    gBattleScripting.battler = battler;
                    ...
                    switch (ability)
                    {
                        ...
                        case ABILITY_TRACE:
                            if (!gSpecialStatuses[battler].switchInAbilityDone)
                            {
                                ...
                                if (IsDoubleBattleForBattler(target))
                                    target = RandomPercentage(RNG_TRACE_TARGET, 50) ? target : BATTLE_PARTNER(target);
                                
                                if (IsBattlerAlive(target) && !gAbilities[gBattleMons[target].ability].cantBeTraced)
                                {
                                    SaveAttackerToStack(battler);
                                ...
                                for (i = 0; i < MAX_BATTLERS_COUNT; i++)
                                {
                                    ...
                                    for (j = 0; j < MAX_MON_MOVES; j++)
                                    {
                                        ...
                                        if (gBattleMoves[move].effect == EFFECT_OHKO)
                                            ++effect;
                                        else if (GetBattleMoveSplit(move) != SPLIT_STATUS)
                                        {
                        ...
                        case ABILITY_FOREWARN:
                            if (!gSpecialStatuses[battler].switchInAbilityDone)
                            {
                                ...
                                {
                                    if (data[i].power > data[bestId].power || (data[i].power == data[bestId].power && RandomPercentage(RNG_FOREWARN, 50)))
                                        bestId = i;
                                }
                                PrepareBattlerNickWithPrefixBuffer(gBattleTextBuff1, data[bestId].battlerId);
                                PrepareMoveBuffer(gBattleTextBuff2, data[bestId].moveId);
                                Free(data);
                        ...
                        case ABILITY_HOSPITALITY:
                            if (!gSpecialStatuses[battler].switchInAbilityDone)
                            {
                                ...
                                if (IsBattlerAlive(partner) && !BATTLER_MAX_HP(partner) && !(gStatuses3[partner] & STATUS3_HEAL_BLOCK))
                                {
                                    gBattleMoveDamage = GetNonDynamaxMaxHP(partner) / 4;
                                    if (gBattleMoveDamage == 0)
                                        gBattleMoveDamage = 1;
                                    gBattleMoveDamage *= -1;
                                    PrepareBattlerNickWithPrefixBuffer(gBattleTextBuff1, partner);
                        ...
                        case ABILITY_MAGMA_ARMOR:
                            ...
                            break;
                        case ABILITY_WATER_VEIL:
                            if (!gSpecialStatuses[battler].switchInAbilityDone && gBattleMons[battler].status1.id == STATUS1_BURN)
                                goto LIMBER_ACTIVATES;
                            break;
                        ...
                        case ABILITY_INTREPID_SWORD:
                            if (CompareStat(battler, STAT_ATK, MAX_STAT_STAGES, CMP_NOT_EQUAL)
                        ...
                        case ABILITY_DAUNTLESS_SHIELD:
                            if (CompareStat(battler, STAT_DEF, MAX_STAT_STAGES, CMP_NOT_EQUAL)
            ...
            case ABILITYEFFECT_ENDTURN:
                switch (ability)
                {
                    case ABILITY_SOLAR_POWER:
                        ...
                        if (GetBattlerWeatherFlags(battler) & B_WEATHER_SUN_ANY)
                        {
                        ...
                        break;
                    case ABILITY_RAIN_DISH:
                        if ((GetBattlerWeatherFlags(battler) & B_WEATHER_RAIN_ANY) && !BATTLER_MAX_HP(battler) && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
                        {
                            RAIN_DISH_SCRIPT:
                    ...
                    case ABILITY_DRY_SKIN:
                        ...
                        break;
                    case ABILITY_ICE_BODY:
                        if ((GetBattlerWeatherFlags(battler) & B_WEATHER_HAIL) && !BATTLER_MAX_HP(battler) && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
                    ...
                    case ABILITY_POISON_HEAL:
                        ...
                        break;
                    case ABILITY_SHED_SKIN:
                        if (RandomWeighted(RNG_SHED_SKIN, 2, 1) && gBattleMons[battler].status1.id)
                        {
                            SHED_SKIN_SCRIPT:
                            ClearBattlerStatus(battler);
                        ...
                        break;
                    case ABILITY_HYDRATION:
                        if (gBattleMons[battler].status1.id && (GetBattlerWeatherFlags(battler) & B_WEATHER_RAIN_ANY))
                            goto SHED_SKIN_SCRIPT;
                        break;
                    case ABILITY_HEALER:
                        if (RandomPercentage(RNG_HEALER, 30) && IsBattlerAlive(BATTLE_PARTNER(battler)) && gBattleMons[BATTLE_PARTNER(battler)].status1.id)
                        {
                            ClearBattlerStatus(BATTLE_PARTNER(battler));
                        ...
                        break;
                    case ABILITY_SPEED_BOOST:
                        if (CompareStat(battler, STAT_SPEED, MAX_STAT_STAGES, CMP_NOT_EQUAL) && gDisableStructs[battler].isFirstTurn != 2
                    ...
                    case ABILITY_SLOW_START:
                        ...
                        break;
                    case ABILITY_HARVEST:
                        if (((GetBattlerWeatherFlags(battler) & B_WEATHER_SUN_ANY) || RandomPercentage(RNG_HARVEST, 50)) && !gBattleMons[battler].item
                    ...
                    case ABILITY_MOODY:
                        if (gDisableStructs[battler].isFirstTurn != 2)
                        {
                            ...
                            {
                                if (CompareStat(battler, i, MIN_STAT_STAGES, CMP_NOT_EQUAL))
                                    validToLower |= Bit(i);
                                
                                if (CompareStat(battler, i, MAX_STAT_STAGES, CMP_NOT_EQUAL))
                                    validToRaise |= Bit(i);
                            }
                            ...
                            if (validToRaise || validToLower)
                            {
                                ...
                                if (validToLower)
                                {
                                    do
                                    {
                                        i = RandomUniform(RNG_MOODY_STAT_DOWN, STAT_ATK, NUM_STATS - 1);
                                    } while (!(validToLower & Bit(i)));
                                    
                                    gBattleCommunication[MULTIUSE_STATE] = i; // Save the stat id to decrease
                                    validToRaise &= ~(Bit(i)); // cannot raise the same stat
                                }
                                ...
                                if (validToRaise)
                                {
                                    do
                                    {
                                        i = RandomUniform(RNG_MOODY_STAT_UP, STAT_ATK, NUM_STATS - 1);
                                    } while (!(validToRaise & Bit(i)));
                    ...
                    case ABILITY_HUNGER_SWITCH:
                    {
                        u32 newSpecies = TryDoBattleFormChange(battler, FORM_CHANGE_ENDTURN);
                    ...
                    case ABILITY_PICKUP:
                    {
                        u32 pickupBattler = GetBattlerOnTopOfPickupStack(battler);
            ...
            case ABILITYEFFECT_UNNERVE:
                ...
                for (i = 0; i < gBattlersCount; i++)
                {
                    ...
                    {
                        u32 abilityId = GetBattlerAbility(i);
                        ...
                        if (abilityId == ABILITY_UNNERVE || abilityId == ABILITY_AS_ONE_ICE_RIDER || abilityId == ABILITY_AS_ONE_SHADOW_RIDER)
                        {
                            ...
                            BattleScriptPushCursorAndCallback(abilityId == ABILITY_UNNERVE ? BattleScript_DisplaySwitchInMsg : BattleScript_AsOneSwitchInActivation);
                            ++effect;
                            break;
                        }
                    }
            ...
            case ABILITYEFFECT_OPPORTUNIST:
                // Is called after ABILITYEFFECT_ON_SWITCHIN to copy any boosts from switch in abilities e.g. intrepid sword.
                for (i = 0; i < gBattlersCount; i++)
                {
                    if (IsBattlerAlive(i) && GetBattlerAbility(i) == ABILITY_OPPORTUNIST && gProtectStructs[i].opportunistState == 2)
                    {
                        gProtectStructs[i].opportunistState--;
                        SaveAttackerToStack(i);
                        BattleScriptPushCursorAndCallback(BattleScript_OpportunistActivates);
                        ++effect;
                        break;
                    }
                }
            ...
            case ABILITYEFFECT_MOVE_END_ATTACKER: // attacker abilities that activates after makes contact with the target
                switch (ability)
                {
                    case ABILITY_STENCH: // Stench check is taken care of in King's Rock check
                        if (RandomPercentage(RNG_STENCH, 10) && IsBattlerAlive(gBattlerTarget) && BattlerTurnDamaged(gBattlerTarget)
                    ...
                    case ABILITY_POISON_TOUCH:
                        if (RandomPercentage(RNG_POISON_TOUCH, 30) && IsBattlerAlive(gBattlerTarget) && BattlerTurnDamaged(gBattlerTarget))
                        {
                    ...
                    case ABILITY_TOXIC_CHAIN:
                        if (RandomPercentage(RNG_TOXIC_CHAIN, 30) && IsBattlerAlive(gBattlerTarget) && BattlerTurnDamaged(gBattlerTarget))
                        {
                ...
                if (!ReceiveSheerForceBoost(gBattlerAttacker, gCurrentMove) || ability == ABILITY_ILLUSION || ability == ABILITY_GULP_MISSILE)
                {
                    switch (ability)
                    {
                        ...
                        case ABILITY_INNARDS_OUT:
                            if (BattlerTurnDamaged(battler) && !IsBattlerAlive(battler) && IsBattlerAlive(gBattlerAttacker)
                            && GetBattlerAbility(gBattlerAttacker) != ABILITY_MAGIC_GUARD)
                            {
                        ...
                        case ABILITY_EFFECT_SPORE:
                            ...
                            {
                                i = RandomUniform(RNG_EFFECT_SPORE, 0, 99);
                        ...
                        case ABILITY_POISON_POINT:
                            ...
                            break;
                        case ABILITY_STATIC:
                            if (!RandomPercentage(RNG_STATIC, 30))
                                break;
                        ...
                        case ABILITY_CUTE_CHARM:
                            if (RandomPercentage(RNG_CUTE_CHARM, 30) && BattlerTurnDamaged(battler) && IsBattlerAlive(gBattlerAttacker) && IsBattlerAlive(battler)
                            && IsMoveMakingContact(gBattlerAttacker, gCurrentMove) && CanBeInfatuatedBy(gBattlerAttacker, battler) == STATUS_CHANGE_WORKED)
                            {
                                SetMoveEffect(MOVE_EFFECT_INFATUATION, TRUE, FALSE);
                                BattleScriptCall(BattleScript_ApplySecondaryEffect);
                                ++effect;
                            }
                            break;
                        ...
                        case ABILITY_WEAK_ARMOR:
                            if (BattlerTurnDamaged(battler) && IsBattlerAlive(battler) && GetBattleMoveSplit(gCurrentMove) == SPLIT_PHYSICAL
                            && (CompareStat(battler, STAT_SPEED, MAX_STAT_STAGES, CMP_NOT_EQUAL) || CompareStat(battler, STAT_DEF, MIN_STAT_STAGES, CMP_NOT_EQUAL)))
                            {
                        ...
                        case ABILITY_LINGERING_AROMA:
                            ...
                            && !gAbilities[gBattleMons[gBattlerAttacker].ability].cantBeSuppressed && GetActiveGimmick(gBattlerAttacker) != GIMMICK_DYNAMAX)
                            {
                        ...
                        case ABILITY_RATTLED:
                            if (BattlerTurnDamaged(battler) && IsBattlerAlive(battler) && CompareStat(battler, STAT_SPEED, MAX_STAT_STAGES, CMP_NOT_EQUAL)
                        ...
                        case ABILITY_STAMINA:
                            if (BattlerTurnDamaged(battler) && IsBattlerAlive(battler) && CompareStat(battler, STAT_DEF, MAX_STAT_STAGES, CMP_NOT_EQUAL))
                            {
                        ...
                        case ABILITY_BERSERK:
                            if (BattlerTurnDamaged(battler) && IsBattlerAlive(battler) && HadMoreThanHalfHpNowDoesnt(battler)
                            && CompareStat(battler, STAT_SPATK, MAX_STAT_STAGES, CMP_NOT_EQUAL) && IS_MULTIHIT_FINAL_STRIKE)
                            {
                        ...
                        case ABILITY_TANGLING_HAIR:
                            if (BattlerTurnDamaged(battler) && IsBattlerAlive(gBattlerAttacker) && IsMoveMakingContact(gBattlerAttacker, gCurrentMove)
                            && CompareStat(gBattlerAttacker, STAT_SPEED, MIN_STAT_STAGES, CMP_NOT_EQUAL))
                            {
                        ...
                        case ABILITY_SAND_SPIT:
                            if (BattlerTurnDamaged(battler) && TryChangeBattleWeather(battler, ENUM_WEATHER_SANDSTORM))
                            {
                        ...
                        case ABILITY_PERISH_BODY:
                            if (BattlerTurnDamaged(battler) && IsBattlerAlive(battler) && IsMoveMakingContact(gBattlerAttacker, gCurrentMove)
                            && !(gStatuses3[gBattlerAttacker] & STATUS3_PERISH_SONG))
                            {
                        ...
                        case ABILITY_WANDERING_SPIRIT:
                            if (BattlerTurnDamaged(battler) && IsBattlerAlive(gBattlerAttacker) && IsMoveMakingContact(gBattlerAttacker, gCurrentMove)
                            && !gAbilities[gBattleMons[gBattlerAttacker].ability].cantBeSwapped && GetActiveGimmick(gBattlerAttacker) != GIMMICK_DYNAMAX
                        ...
                        case ABILITY_GULP_MISSILE:
                            if (BattlerTurnDamaged(battler) && IsBattlerAlive(gBattlerAttacker))
                            {
...
static u8 StatRaiseBerries(u8 battlerId, u8 statId, bool8 moveTurn)
{
    ...
    if (CheckPinchBerryActivate(battlerId, gLastUsedItem) && !moveTurn && CompareStat(battlerId, statId, MAX_STAT_STAGES, CMP_NOT_EQUAL))
    {
    ...
    case 1:
        if (IsBattlerAlive(battlerId))
        {
            ...
            case HOLD_EFFECT_RANDOM_STAT_UP:
                ...
                {
                    for (i = 0; i < 5 && !CompareStat(battlerId, STAT_ATK + i, MAX_STAT_STAGES, CMP_NOT_EQUAL); ++i);
                    if (i != 5)
                    {
                        do
        ...
        if (gBattleMoveDamage && !ReceiveSheerForceBoost(battlerId, gCurrentMove))
        {
            ...
            case HOLD_EFFECT_FLINCH:
                if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
                 ...
                 && RandomPercentage(RNG_HOLD_EFFECT_FLINCH, CalcSecondaryEffectChance(battlerId, MOVE_EFFECT_FLINCH, battlerHoldEffectParam)))
                {
                    SetMoveEffect(MOVE_EFFECT_FLINCH, FALSE, FALSE);
...
bool32 CheckPinchBerryActivate(u32 battler, u32 item)
{
    if (IsUnnerveOnOpposingField(battler) && ItemId_GetPocket(item) == POCKET_BERRY_POUCH)
        return FALSE;
    else if (gBattleMons[battler].hp <= gBattleMons[battler].maxHP / 4 || (GetBattlerAbility(battler) == ABILITY_GLUTTONY && gBattleMons[battler].hp <= gBattleMons[battler].maxHP / 2))
        return TRUE;
    else
        return FALSE;
}
...
{
    u32 opposing = BATTLE_OPPOSITE(battlerId);

    if (!IsDoubleBattleForBattler(opposing))
        return GetBattlerAtPosition(opposing);
    else if (CountAliveMonsInBattle(battlerId, BATTLE_ALIVE_EXCEPT_BATTLER) > 1)
        return GetBattlerAtPosition(RandomUniform(RNG_RANDOM_TARGET, 0, 1) ? opposing : BATTLE_PARTNER(opposing));
    else
        return GetBattlerAtPosition(IsBattlerAlive(opposing) ? opposing : BATTLE_PARTNER(opposing));
}

u32 GetRandomTarget(u32 battlerId)
{
    return GetBattlerAtPosition(sTargetPositions[GetBattlerSide(battlerId)][RandomUniform(RNG_RANDOM_TARGET, 0, 1)]);
}
...
{
    u32 side, moveTarget, moveType, targetBattler;
    ...
    switch (moveTarget)
    {
        ...
        case MOVE_TARGET_SELECTED_OPPONENT:
            ...
            else
            {
                ...
                if (IsMoveAffectedByRedirectionEffects(gBattlerAttacker, move))
                {
                    u32 newTarget;
                    ...
                    {
                        case TYPE_ELECTRIC:
                            if (GetBattlerAbility(targetBattler) != ABILITY_LIGHTNING_ROD && (newTarget = ABILITY_ON_FIELD_EXCEPT_BATTLER(gBattlerAttacker, ABILITY_LIGHTNING_ROD)))
                            {
                                targetBattler = newTarget - 1;
                                gSpecialStatuses[targetBattler].abilityRedirected = TRUE;
                            }
                            break;
                        case TYPE_WATER:
                            if (GetBattlerAbility(targetBattler) != ABILITY_STORM_DRAIN && (newTarget = ABILITY_ON_FIELD_EXCEPT_BATTLER(gBattlerAttacker, ABILITY_STORM_DRAIN)))
                            {
                                targetBattler = newTarget - 1;
                                gSpecialStatuses[targetBattler].abilityRedirected = TRUE;
                            }
...
void CopyMoveTargetName(u32 moveTarget, u8 *dest)
{
    ...
    switch (moveTarget)
    {
        case MOVE_TARGET_SELECTED:
            ...
            break;
        case MOVE_TARGET_SELECTED_OPPONENT:
            str = COMPOUND_STRING("Selected Foe");
            break;
        case MOVE_TARGET_DEPENDS:
        ...
        case MOVE_TARGET_ALLY:
            ...
            break;
        case MOVE_TARGET_FOES_AND_ALLY:
            str = COMPOUND_STRING("Foes and Ally");
            break;
        case MOVE_TARGET_USER_OR_ALLY:
            str = COMPOUND_STRING("User or Ally");
            break;
        case MOVE_TARGET_ALL_BATTLERS:
            str = COMPOUND_STRING("All Field");
            break;
    }
    StringCopyN(dest, str, 14);
}
...
u32 IsMonDisobedient(void)
{
    ...
    if (levelCapLevel == MAX_LEVEL || gBattleMons[gBattlerAttacker].level <= levelCapLevel)
        return 0;
    else if (GetBattlerSide(gBattlerAttacker) == B_SIDE_OPPONENT)
        return 0;
    else if ((gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_POKEDUDE | BATTLE_TYPE_RECORDED)))
        return 0;
    else if (((gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER) && GetBattlerPosition(gBattlerAttacker) == B_POSITION_PLAYER_RIGHT))
        return 0;
    else if (!IsOtherTrainer(gBattleMons[gBattlerAttacker].otId, gBattleMons[gBattlerAttacker].otName))
        return 0;
    else if (gBattleStruct->dancer.inProgress)
        return 0;
    else if (IsMaxMove(gCurrentMove))
        return 0;
    ...
    {
        calc = Bit(gCurrMovePos) | CheckMoveLimitations(gBattlerAttacker, 0);
...
{
    if (battlerId >= gBattlersCount || (gAbsentBattlerFlags & Bit(battlerId)) || GetMonData(GetBattlerPartyIndexPtr(battlerId), MON_DATA_HP) == 0)
        return FALSE;
    
    return TRUE;
}

static void ShouldChangeFormOnWeatherStart(u32 battlerId)
{
    u32 i, side = GetBattlerSide(battlerId);
...
bool32 TryChangeBattleWeather(u32 battlerId, u32 weatherEnumId)
{
    ...
    if ((gBattleWeather & B_WEATHER_PRIMAL_ANY) && ability != ABILITY_PRIMORDIAL_SEA && ability != ABILITY_DESOLATE_LAND && ability != ABILITY_DELTA_STREAM)
        return FALSE;
    else if (!(gBattleWeather & gBattleWeatherInfo[weatherEnumId].flag))
    {
        gBattleWeather = gBattleWeatherInfo[weatherEnumId].flag;
...
u32 GetCurrentWeatherEnumId(u32 weatherFlags)
{
    ...
    for (i = 0; i < ARRAY_COUNT(gBattleWeatherInfo); i++)
    {
        if (weatherFlags & gBattleWeatherInfo[i].flag)
            return i;
    }
...
static inline bool32 IsMoveBlockedByMaxGuard(u32 move)
{
    // Those moves forbidden protection effects, but they are all blocked by Max Guard
    switch (move)
    {
        case MOVE_BLOCK:
    ...
    return !gBattleMoves[move].flags.forbiddenProtect;
}

// Check if defender is protected from attacker's move
// ignoreZMoveCheck is only set to TRUE when calculating Z-Move over protect damage reduction.
bool32 IsBattlerProtected(u32 attacker, u32 defender, u32 move, bool32 ignoreZMoveCheck)
{
    u32 moveTarget, defenderSide;
    
    // Max Guard can protect from moves that forbidden others protection effects
    if (gProtectStructs[defender].maxGuarded && !ignoreZMoveCheck && IsMoveBlockedByMaxGuard(move))
        return TRUE;
    ...
    {
        if (ignoreZMoveCheck || (!IsZMove(move) && !IsMaxMove(move)))
        {
...
bool32 DoSwitchInAbilitiesItems(u32 battlerId)
{
    ...
    || ItemBattleEffects(ITEMEFFECT_ON_SWITCH_IN, battlerId, FALSE) || AbilityBattleEffects(ABILITYEFFECT_OPPORTUNIST, 0))
        return TRUE;
    return FALSE;
}
...
{
    u32 holdEffect, species = gBattleMons[battlerId].species;
...
u32 CountUsablePartyMons(u32 battlerId, u8 *viableMons, bool32 (*excludes)(u32, u32, u32))
{
    ...
    GetBattlerPartyRange(battlerId, &firstMonId, &lastMonId);
...
{
    bool32 hasSereneGrace = (GetBattlerAbility(battlerId) == ABILITY_SERENE_GRACE);
    bool32 sideHasRainbow = (gSideStatuses[GetBattlerSide(battlerId)] & SIDE_STATUS_RAINBOW);
...
static void CopyBattlerCritModifier(u32 attacker, u32 defender)
{
    ...
    gBattleStruct->battlers[attacker].gMaxChiStrikeBoost = gBattleStruct->battlers[defender].gMaxChiStrikeBoost;
}

void CopyBattlerStatChanges(u32 battler1, u32 battler2) // Battler1 copy battler2's stat changes
{
    u32 i;
    ...
    CopyBattlerCritModifier(battler1, battler2);
}
...
u32 GetBattlerHighestStatId(u32 battlerId, bool32 checkWonderRoom)
{
    ...
    for (i = STAT_SPEED; i < NUM_STATS - 1; i++)
    {
        ...
        SWAP(statIds[i], statIds[i + 1], temp);
    }
...
bool32 IsBattlerAffectedByFollowMe(u32 battlerId, u32 opposingSide, u32 move)
{
    if (!gSideTimers[opposingSide].followmeSet || !IsBattlerAlive(gSideTimers[opposingSide].followmeTarget) || (gStatuses3[gSideTimers[opposingSide].followmeTarget] & STATUS3_SKY_DROP_TARGET))
        ...
        return IsMoveAffectedByRedirectionEffects(battlerId, move);
}

// These protects redirection from Lightning Rod/Follow Me.
...
const u8 *PokemonUseItemEffectsBattle(u32 battlerId, u32 itemId, bool32 *canUse)
{
    ...
    u32 i, holdEffectParam = ItemId_GetHoldEffectParam(itemId);
    
    switch (ItemId_GetBattleUsage(itemId))
    {
        case EFFECT_ITEM_INCREASE_STAT:
            if (CompareStat(battlerId, holdEffectParam, MAX_STAT_STAGES, CMP_NOT_EQUAL))
                *canUse = TRUE;
            break;
        ...
        case EFFECT_ITEM_INCREASE_ALL_STATS:
            for (i = STAT_ATK; i < NUM_STATS; i++)
            {
                if (CompareStat(battlerId, i, MAX_STAT_STAGES, CMP_NOT_EQUAL))
                {
                    *canUse = TRUE;
                    break;
                }
...
u32 CountAliveMonsInBattle(u32 battlerId, u32 caseId)
{
    ...
    case BATTLE_ALIVE_SIDE:
        ...
        break;
    case BATTLE_ALIVE_FIELD:
    case BATTLE_ALIVE_EXCEPT_BATTLER:
        for (i = 0; i < MAX_BATTLERS_COUNT; i++)
        {
            if (caseId == BATTLE_ALIVE_EXCEPT_BATTLER && i == battlerId)
                continue;
...
{
    u32 i, money, bonusMoney = 0;
    
    for (i = 0; i < MAX_BATTLERS_COUNT; i++)
    {
        u32 level = gBattleMons[i].level;
        
        money = (gBattleStruct->battlers[i].timesUsedPayDay * level * 5);
        money += (gBattleStruct->battlers[i].timesUsedGMaxGoldRush * level * 100);
...
{
    if (ABILITY_ON_FIELD(ABILITY_CLOUD_NINE) || ABILITY_ON_FIELD(ABILITY_AIR_LOCK))
...
{
    u32 item, type = gBattleMoves[move].type;
    
    if (move != MOVE_STRUGGLE)
    {
        item = gBattleMons[battlerId].item;
        
        // Max Guard is always a Normal type move, regardless of the base move's type or abilities.
        if (GetActiveGimmick(battlerId) == GIMMICK_DYNAMAX && GetBattleMoveSplit(move) == SPLIT_STATUS && !IsRaidBossUsingRegularMove(battlerId, move))
            return TYPE_NORMAL;
        ...
        switch (gBattleMoves[move].effect)
        {
            ...
            case EFFECT_CHANGE_TYPE_ON_ITEM:
                if (GetBattlerItemHoldEffect(battlerId, TRUE) == gBattleMoves[move].argument.generic)
                {
                    ...
                    // Don't change type if Terastallized into a diffent type of the item's hold effect param
                    if (!(GetActiveGimmick(battlerId) == GIMMICK_TERA && GetBattlerTeraType(battlerId) != itemType))
                        type = itemType;
                }
                break;
        }
        
        // Check effects that aren't affected by type change abilities
        if (!GET_MOVE_MOVEEFFECT_TABLE(move).normalizeUnaffected)
        {
...
bool32 TryTransformIntoBattler(u32 battler1, u32 battler2)
{
    ...
    u32 i, targetSpecies, oldAbility;
    
    if ((gBattleMons[battler2].status2 & (STATUS2_TRANSFORMED | STATUS2_SUBSTITUTE)) || (gStatuses3[battler2] & STATUS3_SEMI_INVULNERABLE))
        return FALSE;
    else if (gBattleMons[battler1].status2 & STATUS2_TRANSFORMED)
        return FALSE;
    else if (gBattleStruct->battlers[battler1].illusionState == ILLUSION_STATE_ON)
        return FALSE;
    else if (gBattleStruct->battlers[battler2].illusionState == ILLUSION_STATE_ON)
        return FALSE;
    
    switch (GetActiveGimmick(battler1))
    {
        case GIMMICK_DYNAMAX:
            if (IsBannedSpeciesForDynamaxing(gBattleMons[battler2].species))
                return FALSE;
            break;
        case GIMMICK_TERA:
            if (GetBattlerTeraType(battler1) == TYPE_STELLAR)
                return FALSE;
            break;
    }
    ...
    if (IsGigantamaxed(battler2))
        targetSpecies = gBattleMonForms[GetBattlerSide(battler2)][gBattlerPartyIndexes[battler2]]; // Copy the base species
    else
        targetSpecies = gBattleMons[battler2].species;

    PrepareSpeciesBuffer(gBattleTextBuff1, targetSpecies);
    gBattleMons[battler1].species = targetSpecies;
        ...
        switch (GetBattlerMoveTargetType(attacker, move))
        {
            case MOVE_TARGET_BOTH:
                if (IS_WHOLE_SIDE_ALIVE(defender)) // Both foes alive to be attacked
                    return FALSE;
                else if (checkTargetsDone && gBattleStruct->battlers[attacker].targetsDone & Bit(BATTLE_PARTNER(defender))) // Already did damage to one foe
                    return FALSE;
                break;
            case MOVE_TARGET_FOES_AND_ALLY:
                if (CountAliveMonsInBattle(attacker, BATTLE_ALIVE_EXCEPT_BATTLER) >= 2) // Both foes or ally alive to be attacked
                    return FALSE;
                else if (checkTargetsDone && ((gBattleStruct->battlers[attacker].targetsDone & Bit(BATTLE_PARTNER(defender)))
                    || (gBattleStruct->battlers[attacker].targetsDone & Bit(BATTLE_PARTNER(attacker))))) // Already did damage at least one target
                    return FALSE;
                break;
        }
    }
...
bool32 TryRemoveEntryHazards(u32 battlerId, bool32 clear, bool32 fromBothSides)
{
    u32 side, i = 0;
    bool32 removed = FALSE;
    ...
    do
    {
        ...
        if (FindQueuedEffectsInSideList(side, gEntryHazardsQueuedEffectIds) != B_SIDE_QUEUED_COUNT)
        {
            if (clear)
            {
                ...
                RemoveBattleEffectFromSideQueueList(side, B_SIDE_QUEUED_GMAX_STEELSURGE);
...
bool32 CanUseLastResort(u32 battlerId)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if (move == gCurrentMove)
            knowsMove = TRUE;
        else if (!(gDisableStructs[battlerId].usedMoveIndices & Bit(i)))
            return FALSE;
    }
...
{
    if (gBattleTypeFlags & (BATTLE_TYPE_RECORDED | BATTLE_TYPE_RAID | BATTLE_TYPE_LINK))
        return TRUE;
    else if (!FlagGet(FLAG_SYS_BAG_ENABLED))
        return TRUE;
...
bool32 MoveHasMoveEffectWithChance(u32 move, u32 moveEffect, u32 chance)
{
    ...
    for (i = 0; i < gBattleMoves[move].numAdditionalEffects; i++)
    {
        if (gBattleMoves[move].additionalEffects[i].moveEffect == moveEffect && gBattleMoves[move].additionalEffects[i].chance == chance)
            return TRUE;
    }
    return FALSE;
}
...
{
    u32 i, chain = gBattleStruct->sos.calls;
...
static u32 CalcSosBattleCallRate(u32 battlerCaller)
{
    u32 callRate = gSpeciesInfo[gBattleMons[battlerCaller].species].sosCallRate;
...
static u32 GetSosBattleCallRateAnswer(u32 battlerCaller, u32 playerBattler, u32 callRate)
{
    u32 answerRate;
...
static u32 GetSosBattleSpeciesToCall(u32 battlerCaller)
{
    u32 species, callerSpecies = gBattleMons[battlerCaller].species;
...
{
    u32 newSlotPartyId = gBattlerPartyIndexes[ally];
    ...
    ClearTemporarySpeciesSpriteData(ally, 0);
...
bool32 TryInitSosCall(void)
{
    ...
    if (caller != MAX_BATTLERS_COUNT)
    {
        ...
        if (!gBattleMons[caller].status1.id && !(gStatuses3[caller] & (STATUS3_SEMI_INVULNERABLE | STATUS3_COMMANDING)))
        {
            ...
            if (RandomPercentage(RNG_SOS_CALL_RATE, callRate))
            {
                u32 playerBattler = GetBattlerAtPosition(B_POSITION_PLAYER_LEFT);
                u32 species = GetSosBattleSpeciesToCall(caller);
                
                gBattleStruct->sos.lastCallBattler = gEffectBattler = caller;
                gBattleScripting.battler = gBattlerFainted = ally;
                
                // Answer failed
                if (!species || !RandomPercentage(RNG_SOS_CALL_ANSWER, GetSosBattleCallRateAnswer(caller, playerBattler, callRate)))
                {
...
void SwapBattlersPositions(u32 battler1, u32 battler2)
{
    ...
    SwapStructData(&gQuickClawTurnRandom[battler1], &gQuickClawTurnRandom[battler2], data, sizeof(struct QuickClawRandom));
    ...
    SwapStructData(&gAIData->thinking[battler1].targets[battler1], &gAIData->thinking[battler1].targets[battler2], data, sizeof(struct AITarget));
    SwapStructData(&gAIData->thinking[battler2].targets[battler2], &gAIData->thinking[battler2].targets[battler1], data, sizeof(struct AITarget));
    SwapStructData(&gAIData->thinking[battler1], &gAIData->thinking[battler2], data, sizeof(struct AIThinking));
    ...
    // These are slot based and can't be swapped, so swap then back here
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        ...
        // Swap battler on pickup stack
        if (gBattleStruct->pickupStack[i] != 0xFF && IsBattlerAlly(battler1, gBattleStruct->pickupStack[i]))
            SWAP_FIELD_BATTLER_ID(gBattleStruct->pickupStack[i]);
    }
...
bool32 TryBattleChallengeStartingStatus(void)
{
    ...
    switch (gBattleStruct->battleChallenge)
    {
        case TRAINER_CHALLENGE_INFINITE_TAILWIND:
            ...
            break;
        case TRAINER_CHALLENGE_INFINITE_MIST:
            if (!(gSideStatuses[B_SIDE_OPPONENT] & SIDE_STATUS_MIST))
            {
                gSideStatuses[B_SIDE_OPPONENT] |= SIDE_STATUS_MIST;
                gSideTimers[B_SIDE_OPPONENT].mistTimer = 0;
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_MIST_SHROUDED_TRAINER_TEAM;
                gBattleScripting.animArg1 = B_ANIM_MIST;
                effect = TRUE;
            }
            break;
        case TRAINER_CHALLENGE_INFINITE_SAFEGUARD:
            if (!(gSideStatuses[B_SIDE_OPPONENT] & SIDE_STATUS_SAFEGUARD))
            {
                gSideStatuses[B_SIDE_OPPONENT] |= SIDE_STATUS_SAFEGUARD;
                gSideTimers[B_SIDE_OPPONENT].safeguardTimer = 0;
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_VEIL_CLOAKED_TRAINER_TEAM;
                gBattleScripting.animArg1 = B_ANIM_SAFEGUARD;
                effect = TRUE;
            }
            break;
        case TRAINER_CHALLENGE_INFINITE_TRICK_ROOM:
            if (!(gFieldStatus & STATUS_FIELD_TRICK_ROOM))
            {
                gFieldStatus |= STATUS_FIELD_TRICK_ROOM;
                gFieldTimers.trickRoomTimer = 0;
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_TWISTED_DIMENSIONS_PERMANENTLY;
                gBattleScripting.animArg1 = B_ANIM_TRICK_ROOM;
                effect = TRUE;
            }
            break;
        case TRAINER_CHALLENGE_INFINITE_GRAVITY:
    ...
    if (effect)
        SaveAttackerToStack(GetBattlerAtPosition(B_POSITION_OPPONENT_LEFT));
...
bool32 TryPrimalReversion(u32 battler)
{
    ...
    if (targetSpecies)
    {
        ...
        gLastUsedItem = gBattleMons[battler].item;
        gBattleScripting.animArg1 = ItemId_GetHoldEffectParam(gLastUsedItem);
        gBattleScripting.battler = battler;
