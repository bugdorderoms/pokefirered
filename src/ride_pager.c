...
static void Task_InitRidePager(u32 taskId);
static void CreateRidePagerMultichoiceWindow(u32 taskId);
...
static void DestroyRidePagerWindow(u32 taskId, bool32 useRide);
static void SharpedoPaddleCallback(u32 taskId);
static void CharizardGlideCallback(u32 taskId);
static void PlayerToPokeRideCallback(u32 taskId);
static void Task_CheckPlayerMovementAndUseRide(u32 taskId);
static void Task_SummonMonWaitPlayerAnim(u32 taskId);
...
static void Task_RidePagerHandlePicboxUpdate(u32 taskId)
{
    s16 *data = gTasks[taskId].data;
...
static void DestroyRidePagerWindow(u32 taskId, bool32 useRide)
{
    ...
    if (useRide)
    {
        ...
        gTasks[taskId].func = sPokeRidesTable[tSelectedId - 1].action.func.void_u32;
    }
    else
    {
        PlaySE(SE_WIN_OPEN);
...
static void Task_CharizardGlide_OpenMap(u32 taskId)
{
...
static u32 GetRideMovementInput(u32 *newDirection);
static u32 RideInputHandler_Normal(u32 *newDirection);
static u32 RideInputHandler_Turning(u32 *newDirection);
...
static u32 (*const sRideInputHandlers[])(u32 *) =
{
...
{
    u32 input = GetRideMovementInput(&newDirection);
