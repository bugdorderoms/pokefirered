...
static void CB2_ReturnToPartyMenuFromMoveDelete(void)
{
	...
	gSpecialVar_0x8005 == MAX_MON_MOVES ? Task_TryCreateSelectionWindow : Task_DisplayMoveDeleted, gPartyMenu.exitCallback);
}
...
void ItemUseCB_TMHM(u8 taskId, UNUSED TaskFunc func)
{
    ...
    StringCopy(gStringVar2, gBattleMoves[move[0]].name);
    move[1] = 0;
...
void ItemUseCB_Fusion(u8 taskId, TaskFunc func)
{
	...
	else if ((masterSpecies = GetFusionData(1, species)) && GetMonData(GetBaseMonForFusedSpecies(), MON_DATA_SPECIES)) // try defuse
	{
		if (gPlayerPartyCount == PARTY_SIZE)
		{
			...
			DoItemUseFormChange(taskId, mon, masterSpecies, ItemId_GetHoldEffectParam(gSpecialVar_ItemId), 2);
	}
	else
	{
		gPartyMenuUseExitCallback = FALSE;
