...
static void PokeSum_RemoveWindows(u32 curPageIndex);
static void Task_PokeSum_FlipPages(u32 taskId);
static void Task_FlipPages_FromInfo(u32 taskId);
static void Task_PokeSum_SwitchDisplayedPokemon(u32 taskId);
static void PokeSum_SeekToNextMon(s8 direction);
static void Task_DestroyResourcesOnExit(u32 taskId);
static void PokeSum_HideSpritesBeforePageFlip(void);
static void PokeSum_ShowSpritesBeforePageFlip(void);
static void PokeSum_UpdateWin1ActiveFlag(u32 curPageIndex);
static void PokeSum_AddWindows(u32 curPageIndex);
static void PokeSum_PrintPageHeaderText(u32 curPageIndex);
static void PokeSum_InitBgCoordsBeforePageFlips(void);
static bool32 PokeSum_IsPageFlipFinished(void);
static void Task_HandleInput_SelectMove(u32 id);
...
static void PokeSum_UpdateBgPriorityForPageFlip(bool32 setBg0Priority, bool32 keepBg1Bg2PriorityOrder);
static void ShowOrHideHpBarObjs(bool32 invisible);
static void ShowOrHideExpBarObjs(bool32 invisible);
static void PokeSum_ShowOrHideMonPicSprite(bool32 invisible);
static void PokeSum_ShowOrHideMonMarkingsSprite(bool32 invisible);
static void ShowOrHideBallIconObj(bool32 invisible);
static void ShowOrHideStatusIcon(bool32 invisible);
static void HideShowPokerusIcon(bool32 invisible);
static void HideShowShinyStar(bool32 invisible);
static void ShoworHideMoveSelectionCursor(bool32 invisible);
static void PokeSum_ShowOrHideMonIconSprite(bool32 invisible);
static void PokeSum_Setup_InitGpu(void);
static void PokeSum_Setup_SpritesReset(void);
static bool32 PokeSum_HandleLoadBgGfx(void);
static bool32 PokeSum_HandleCreateSprites(void);
static void PokeSum_CreateWindows(void);
static bool32 PokeSum_Setup_BufferStrings(void);
...
static u32 StatusToAilment(u32 statusId);
static void BufferMonMoveI(u32);
static void CreateShinyStarObj(void);
static void CreatePokerusIconObj(u32, u32);
static void PokeSum_CreateMonMarkingsSprite(void);
static void CreateMoveSelectionCursorObjs(u32, u32);
static void CreateMonStatusIconObj(u32, u32);
static void CreateHpBarObjs(u32, u32);
static void CreateExpBarObjs(u32, u32);
...
static void Task_InputHandler_SelectOrForgetMove(u32 taskId);
...
static void PokeSum_PrintMoveName(u32 i);
...
static bool32 MapSecIsInKantoOrSevii(u32 mapSec);
...
static bool32 PokeSum_BufferOtName_IsEqualToCurrentOwner(struct Pokemon * mon);
...
static void PokeSum_SetMonPicSpriteCallback(u32 spriteId);
...
static const u8 sText_Somewhere[] = _("Somewhere");
...
static const u8 sText_PokeSum_EggOrigin_DayCare[] = _("An odd Pokémon Egg found by the\nDay-Care couple.");
static const u8 sText_PokeSum_EggOrigin_Trade[] = _("A peculiar Pokémon Egg obtained\nin a trade.");
static const u8 sText_PokeSum_EggOrigin_TravelingMan[] = _("An odd Pokémon Egg obtained from\nthe traveling man.");
static const u8 sText_PokeSum_EggOrigin_NicePlace[] = _("A wondrously peculiar Pokémon Egg\nobtained at a nice place.");
static const u8 sText_PokeSum_EggOrigin_Spa[] = _("An odd Pokémon Egg obtained from\nan old woman at the Spa.");

static const u8 sText_PokeSum_EggHatch_LongTime[] = _("It looks like this\nEgg will take a\nlong time to hatch.");
static const u8 sText_PokeSum_EggHatch_SomeTime[] = _("What will hatch\nfrom this? It will\ntake some time.");
static const u8 sText_PokeSum_EggHatch_Soon[] = _("It occasionally\nmoves. It should\nhatch soon.");
static const u8 sText_PokeSum_EggHatch_AlmostReady[] = _("It's making sounds.\nIt's almost ready\nto hatch!");
...
static const u8 * const sEggHatchTimeTexts[] = {
    sText_PokeSum_EggHatch_LongTime,
    sText_PokeSum_EggHatch_SomeTime,
    sText_PokeSum_EggHatch_Soon, 
    sText_PokeSum_EggHatch_AlmostReady
};

static const u8 * const sEggOriginTexts[] = {
    sText_PokeSum_EggOrigin_DayCare,      
    sText_PokeSum_EggOrigin_Trade,
    sText_PokeSum_EggOrigin_TravelingMan, 
    sText_PokeSum_EggOrigin_Trade,
    sText_PokeSum_EggOrigin_NicePlace,
    sText_PokeSum_EggOrigin_Spa,
    sText_PokeSum_EggOrigin_Trade
};
...
void ShowPokemonSummaryScreen(struct Pokemon * party, u32 cursorPos, u32 lastIdx, MainCallback savedCallback, u32 mode)
{
...
void ShowSelectMovePokemonSummaryScreen(struct Pokemon * party, u32 cursorPos, u32 lastIdx, MainCallback savedCallback, u32 a4)
{
...
static bool32 PageFlipInputIsDisabled(u32 direction)
{
...
bool32 IsPageFlipInput(u32 direction)
{
...
static void Task_InputHandler_Info(u32 taskId)
{
...
static void Task_FlipPages_FromInfo(u32 taskId)
{
    ...
    case 3:
        ...
        PokeSum_PrintControlsString((gMain.inBattle || gReceivedRemoteLinkPlayers) ? sText_PokeSum_Controls_Pick : sText_PokeSum_Controls_PickSwitch);
        break;
    case 4:
        CopyWindowToVram(sMonSummaryScreen->windowIds[POKESUM_WIN_PAGE_NAME], 2);
        CopyWindowToVram(sMonSummaryScreen->windowIds[POKESUM_WIN_CONTROLS], 2);
...
static void Task_BackOutOfSelectMove(u32 taskId)
{
    switch (sMonSummaryScreen->state3284)
    {
    case 0:
    ...
    case 4:
        PokeSum_PrintPageName(sText_PokeSum_PageName_KnownMoves);
        PokeSum_PrintControlsString(sText_PokeSum_Controls_PageDetail);
        break;
    case 5:
        CopyWindowToVram(sMonSummaryScreen->windowIds[POKESUM_WIN_PAGE_NAME], 2);
        CopyWindowToVram(sMonSummaryScreen->windowIds[POKESUM_WIN_CONTROLS], 2);
...
static void PokeSum_InitBgCoordsBeforePageFlips(void)
{
    ...
    {
        PokeSum_UpdateBgPriorityForPageFlip(FALSE, FALSE);
    ...
    if (sMonSummaryScreen->pageFlipDirection == 1)
    {
        ...
        SetGpuReg(REG_OFFSET_BG1HOFS, 0);
        SetGpuReg(REG_OFFSET_BG2HOFS, 0);
        PokeSum_UpdateBgPriorityForPageFlip(TRUE, TRUE);
    }
    else
    {
        ...
        PokeSum_UpdateBgPriorityForPageFlip(TRUE, FALSE);
    }
...
static void BufferMonInfo(void)
{
    u32 heldItem, species = GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_SPECIES);
    u8 tempStr[20];
    u32 gender, otId;
    ...
    else
    {
        ...
        if (gender == MON_FEMALE)
            ...
            StringCopy(sMonSummaryScreen->summary.genderSymbolStrBuf, COMPOUND_STRING(""));
        ...
        StringCopy(sMonSummaryScreen->summary.levelStrBuf, COMPOUND_STRING("{LV_2}"));
        ...
        if (heldItem == ITEM_NONE)
            StringCopy(sMonSummaryScreen->summary.itemNameStrBuf, COMPOUND_STRING("None"));
        else
...
static void BufferMonSkills(void)
{
    ...
    StringAppend(sMonSummaryScreen->summary.curHpStrBuf, COMPOUND_STRING("/"));
...
static void BufferMonMoveI(u32 i)
{
    ...
    {
        sMonSummaryScreen->summary.moveNameStrBufs[i] = COMPOUND_STRING("-");
        StringCopy(sMonSummaryScreen->summary.moveCurPpStrBufs[i], COMPOUND_STRING("--"));
        StringCopy(sMonSummaryScreen->summary.movePowerStrBufs[i], gText_MoveNoPowerAccuracy);
        StringCopy(sMonSummaryScreen->summary.moveAccuracyStrBufs[i], gText_MoveNoPowerAccuracy);
        sMonSkillsPrinterXpos->curPp[i] = 0xFF;
        sMonSkillsPrinterXpos->maxPp[i] = 0xFF;
    }
    else
    {
        u32 move = sMonSummaryScreen->moveIds[i];
...
static void PrintInfoPage(void)
{
    ...
    {
        u32 hatchMsgIndex, eggCycles = GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_FRIENDSHIP);
...
static u32 GetStatColor(u32 statId)
{
#if NATURE_COLOURS
    u32 nature = GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_NATURE);
    ...
    if (sMonSummaryScreen->curPageIndex == PSS_PAGE_MOVES_INFO)
    {
        if (sMonSummaryScreen->mode == PSS_MODE_SELECT_MOVE)
            ...
            AddTextPrinterParameterized3(sMonSummaryScreen->windowIds[POKESUM_WIN_RIGHT_PANE], 2, 3, GetMoveNamePrinterYpos(MAX_MON_MOVES),
                                         sPrintMoveTextColors[0], TEXT_SPEED_FF, COMPOUND_STRING("Cancel"));
    }
}
...
{
    u32 colorIdx = 0;
    u32 curPP = GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_PP1 + i);
    u32 maxPP = CalculatePPWithBonus(sMonSummaryScreen->moveIds[i], GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_PP_BONUSES), i);
    ...
    AddTextPrinterParameterized3(sMonSummaryScreen->windowIds[POKESUM_WIN_RIGHT_PANE], 2, 36, GetMovePpPinterYpos(i), sPrintMoveTextColors[colorIdx], TEXT_SPEED_FF, COMPOUND_STRING("{PP}"));
    ...
    if (sMonSummaryScreen->moveIds[i])
    {
        AddTextPrinterParameterized3(sMonSummaryScreen->windowIds[POKESUM_WIN_RIGHT_PANE], 2, 58, GetMovePpPinterYpos(i), sPrintMoveTextColors[colorIdx], TEXT_SPEED_FF, COMPOUND_STRING("/"));
...
static void PokeSum_PrintTrainerMemo_Mon_HeldByOT(void)
{
    ...
    else
    {
        if (sMonSummaryScreen->isEnemyParty || IsMultiBattlePartner())
            StringCopy(mapNameStr, sText_Somewhere);
        else
...
static void PokeSum_PrintTrainerMemo_Mon_NotHeldByOT(void)
{
    ...
    if (!MapSecIsInKantoOrSevii(metLocation) || !CurrentMonIsFromGBA())
    {
        ...
        {
            DynamicPlaceholderTextUtil_ExpandPlaceholders(natureMetOrHatchedAtLevelStr, metLocation == METLOC_SPECIAL_ENCOUNTER ? sText_PokeSum_FatefulEncounterMet : sText_PokeSum_MetInATrade);
            AddTextPrinterParameterized4(sMonSummaryScreen->windowIds[POKESUM_WIN_TRAINER_MEMO], 2, 0, 3, 0, 0, sLevelNickTextColors[0], TEXT_SPEED_FF, natureMetOrHatchedAtLevelStr);
        }
        return;
    }
...
static void PokeSum_PrintTrainerMemo_Egg(void)
{
    u32 metLocation = GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_MET_LOCATION);
    u32 version;
    u32 chosenStrIndex = 0;
...
static void PokeSum_PrintPageHeaderText(u32 curPageIndex)
{
    ...
    {
    case PSS_PAGE_INFO:
        PokeSum_PrintPageName(sText_PokeSum_PageName_PokemonInfo);
        PokeSum_PrintControlsString(sMonSummaryScreen->isEgg ? sText_PokeSum_Controls_Cancel : sText_PokeSum_Controls_PageCancel);
        PrintMonLevelNickOnWindow2(sText_PokeSum_NoData);
        break;
    case PSS_PAGE_SKILLS:
        PokeSum_PrintPageName(sText_PokeSum_PageName_PokemonSkills);
        PokeSum_PrintControlsString(sText_PokeSum_Controls_Page);
        PrintMonLevelNickOnWindow2(sText_PokeSum_NoData);
        break;
    case PSS_PAGE_MOVES:
        PokeSum_PrintPageName(sText_PokeSum_PageName_KnownMoves);
    ...
    case PSS_PAGE_MOVES_INFO:
        PokeSum_PrintPageName(sText_PokeSum_PageName_KnownMoves);
        PokeSum_PrintControlsString(gMain.inBattle ? sText_PokeSum_Controls_Pick : sText_PokeSum_Controls_PickSwitch);
        PrintMonLevelNickOnWindow2(sText_PokeSum_NoData);
        break;
    case PSS_PAGE_MOVE_DELETER:
        PokeSum_PrintPageName(sText_PokeSum_PageName_KnownMoves);
        PokeSum_PrintControlsString(sText_PokeSum_Controls_PickDelete);
        PrintMonLevelNickOnWindow2(sText_PokeSum_NoData);
        break;
    default:
        break;
...
void SetPokemonSummaryScreenMode(u32 mode)
{
    sMonSummaryScreen->mode = mode;
}
...
static void Task_HandleInput_SelectMove(u32 taskId)
{
    u32 i;
...
static void PokeSum_CreateMonIconSprite(void)
{
    u32 species = GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_SPECIES2);
...
static void CreateMoveSelectionCursorObjs(u32 tileTag, u32 palTag)
{
    u32 i;
...
static void CreateMonStatusIconObj(u32 tileTag, u32 palTag)
{
    void * gfxBufferPtr;
...
static void CreateHpBarObjs(u32 tileTag, u32 palTag)
{
    u32 i;
    void * gfxBufferPtr;
    u32 curHp;
    u32 maxHp;
    u32 hpBarPalTagOffset;
...
static void CreateExpBarObjs(u32 tileTag, u32 palTag)
{
    u32 i;
    void * gfxBufferPtr;
...
static void CreatePokerusIconObj(u32 tileTag, u32 palTag)
{
    void * gfxBufferPtr;
...
static bool32 PokeSum_CanSeekToMon(struct Pokemon * partyMons)
{
...
static s8 SeekToMonInMultiParty_SeekForward(u32 startingIdx)
{
    while (TRUE)
    {
...
static s8 SeekToMonInMultiParty_SeekBack(u32 startingIdx)
{
    while (1)
    {
        if (startingIdx == 0)
...
static bool32 CurrentMonIsFromGBA(void)
{
    u32 version = GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_MET_GAME);
