...
static void ListMenuScroll(struct ListMenu *list, u8 count, bool8 movingDown)
{
    ...
    else
    {
        ...
        if (!movingDown)
        {
            ...
            FillWindowPixelRect(list->template.windowId, PIXEL_FILL(list->template.fillValue), 0, y, GetWindowAttribute(list->template.windowId, WINDOW_WIDTH) * 8, height);
        }
        else
        {
...
static bool8 ListMenuChangeSelection(struct ListMenu *list, bool8 updateCursorAndCallCallback, u8 count, bool8 movingDown)
{
    ...
    u8 i, selectionChange = 0, cursorCount = 0;
