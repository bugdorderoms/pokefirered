...
void BattleAI_HandleItemUseBeforeAISetup(u8 battlerId)
{
    ...
    if ((gBattleTypeFlags & BATTLE_TYPE_TRAINER) && !(gBattleTypeFlags & (BATTLE_TYPE_SAFARI | BATTLE_TYPE_LINK)))
    {
...
static void Cmd_if_user_has_attacking_move(void)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (gBattleMons[gBattlerAttacker].moves[i] != 0 && gBattleMoves[gBattleMons[gBattlerAttacker].moves[i]].power != 0)
            break;
    }
...
static void Cmd_if_can_faint(void)
{
    ...
    AI_CalcDmg(gBattlerAttacker, gBattlerTarget, AI_THINKING_STRUCT->moveConsidered);
...
static void Cmd_get_hold_effect(void)
{
    ...
    if (GetBattlerSide(battlerId) == B_SIDE_PLAYER)
    {
        ...
        AI_THINKING_STRUCT->funcResult = GetBattlerItemHoldEffect(battlerId, TRUE);
