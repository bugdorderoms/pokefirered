...
static void OakSpeechNidoranFFreeResources(void);
static void *OakSpeechNidoranFGetBuffer(u32 bufferId);
static void DestroyLinkedPikaOrGrassPlatformSprites(u32 taskId, u32 state);
static void LoadOaksSpeechTrainerPic(u32 whichPic, u32 tileOffset);
static void DestroyOaksSpeechTrainerPic(void);
static void CreateFadeInTask(u32 taskId, u32 state);
static void CreateFadeOutTask(u32 taskId, u32 state);
...
static void CreateHelpDocsPage1(void)
{
    ...
    AddTextPrinterParameterized4(sOakSpeechResources->windowIds[0], 2, 2, 0, 1, 1, sTextColor_HelpSystem, 0, gNewGame_HelpDocs1);
...
static void Task_OakSpeech4(u32 taskId)
{
    ...
    else
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            PutWindowTilemap(sOakSpeechResources->windowIds[i]);
...
static void Task_OakSpeech6(u32 taskId)
{
    ...
    else
    {
        ...
        CopyBgTilemapBufferToVram(1);
        FREE_AND_SET_NULL(sOakSpeechResources->pikachuIntroTilemap);
...
static void Task_OakSpeech19(u32 taskId)
{
    if (!IsTextPrinterActive(0))
    {
        ...
        DrawStdFrameWithCustomTileAndPalette(gTasks[taskId].data[13], 1, STD_WINDOW_BASE_TILE_NUM, 14);
...
static void Task_OakSpeech25(u32 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        else
        {
            ...
            DoNamingScreen(NAMING_SCREEN_RIVAL, gSaveBlock1Ptr->rivalName, 0, 0, CB2_ReturnFromNamingScreen);
        }
...
static void Task_OakSpeech26(u32 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        else if (!IsTextPrinterActive(0))
        {
            ...
            {
                CreateYesNoMenu(&sNewGameAdventureIntroWindowTemplates[2], 2, 0, 2, STD_WINDOW_BASE_TILE_NUM, 14, 0);
...
static struct OakSpeechNidoranFStruct *OakSpeechNidoranFSetup(void)
{
    ...
    else
    {
        do
        {
            for (i = 0; i < (s8)gOakSpeechNidoranResources->spriteCount; ++i)
                gOakSpeechNidoranResources->bufferPtrs[i] = &gOakSpeechNidoranResources->dataBuffer[gOakSpeechNidoranResources->sizePerSprite * i];
        } while (0);
    }
    ...
    if (flags)
    {
        ...
        gOakSpeechNidoranResources->enable = 0xA3;

    return gOakSpeechNidoranResources;
}
...
static void *OakSpeechNidoranFGetBuffer(u32 bufferId)
{
    ...
    {
        if (bufferId >= (s8)gOakSpeechNidoranResources->spriteCount)
            bufferId = 0;
        return gOakSpeechNidoranResources->bufferPtrs[bufferId];
    }
}
