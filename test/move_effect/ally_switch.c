...
DOUBLE_BATTLE_TEST("Ally Switch changes the position of battlers")
{
    GIVEN {
        ...
        MESSAGE("Wynaut's Defense sharply fell!");
...
DOUBLE_BATTLE_TEST("Ally Switch has no effect on partner's chosen move")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WYNAUT) { Moves(MOVE_SCRATCH, MOVE_POUND, TEST_DEFAULT_MOVE, MOVE_SCRATCH); }
...
{
    GIVEN {
        ASSUME(gBattleMoves[MOVE_HARDEN].target == MOVE_TARGET_USER);
        ...
        EXPECT_EQ(playerLeft->statStages[STAT_DEF], DEFAULT_STAT_STAGES + 1);
    }
}
...
DOUBLE_BATTLE_TEST("Ally Switch works if ally used two-turn move like Dig")
{
    GIVEN {
        ...
        MESSAGE("Wynaut burrowed its way under the ground!");
...
DOUBLE_BATTLE_TEST("Ally Switch swaps Illusion data")
{
    GIVEN {
        ...
        EXPECT(&gPlayerParty[2] == gBattleStruct->battlers[B_POSITION_PLAYER_LEFT].illusion.mon);
    }
}
