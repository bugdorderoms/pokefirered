...
static void BattleTest_SetUp(void *data)
{
    ...
    STATE = AllocZeroed(sizeof(*STATE));
    if (!STATE)
        Test_ExitWithResult(TEST_RESULT_ERROR, SourceLine(0), "OOM: STATE = AllocZerod(%d)", sizeof(*STATE));
    InvokeTestFunction(test);
    STATE->parameters = STATE->parametersCount;
    STATE->results = AllocZeroed(test->resultsSize * STATE->parameters);
    if (!STATE->results)
        Test_ExitWithResult(TEST_RESULT_ERROR, SourceLine(0), "OOM: STATE->results = AllocZerod(%d)", sizeof(test->resultsSize * STATE->parameters));
    
    switch (test->type)
    {
    case BATTLE_TEST_SINGLES:
    case BATTLE_TEST_WILD:
        STATE->battlersCount = MAX_BATTLERS_COUNT / 2;
        break;
    case BATTLE_TEST_DOUBLES:
    case BATTLE_TEST_WILD_DOUBLE:
...
static void SetImplicitSpeeds(void)
{
    ...
    while (hasSpeeds != allSpeeds)
    {
        ...
        {
            if (!(hasSpeeds & (1 << i)) && !(DATA.slowerThan[B_SIDE_PLAYER][i] & ~hasSpeeds))
            {
        ...
        {
            if (!(hasSpeeds & ((1 << 6) << i)) && !(DATA.slowerThan[B_SIDE_OPPONENT][i] & ~hasSpeeds))
            {
...
static void BattleTest_Run(void *data)
{
    ...
    {
        case BATTLE_TEST_SINGLES:
            DATA.recordedBattle.battleFlags = BATTLE_TYPE_TRAINER | BATTLE_TYPE_IS_MASTER;
            break;
        case BATTLE_TEST_DOUBLES:
            DATA.recordedBattle.battleFlags = BATTLE_TYPE_TRAINER | BATTLE_TYPE_IS_MASTER | BATTLE_TYPE_DOUBLE;
            break;
        case BATTLE_TEST_WILD:
            DATA.recordedBattle.battleFlags = BATTLE_TYPE_IS_MASTER;
            break;
        case BATTLE_TEST_WILD_DOUBLE:
            DATA.recordedBattle.battleFlags = BATTLE_TYPE_IS_MASTER | BATTLE_TYPE_DOUBLE;
            break;
    }
    ...
    {
        if (DATA.explicitSpeeds[B_SIDE_PLAYER] != (1 << DATA.playerPartySize) - 1 && DATA.explicitSpeeds[B_SIDE_OPPONENT] != (1 << DATA.opponentPartySize) - 1)
        {
...
u32 RandomUniform(u32 randomTag, u32 lo, u32 hi)
{
    ...
    if (randomTag == STATE->rngTag && CanCheckRngTag())
    {
        ...
        {
            Test_ExitWithResult(TEST_RESULT_ERROR, SourceLine(0), "RandomUniform called with inconsistent trials %d and %d", STATE->trials, n);
        }
...
u32 RandomWeightedArray(u32 randomTag, u32 sum, u32 n, const u8 *weights)
{
    ...
    if (randomTag == STATE->rngTag && CanCheckRngTag())
    {
        ...
        {
            Test_ExitWithResult(TEST_RESULT_ERROR, SourceLine(0), "RandomWeighted called with tag %d has inconsistent trials %d and %d", randomTag, STATE->trials, n);
        }
    ...
    default:
        ...
        {
            if (n == 1)
                Test_ExitWithResult(TEST_RESULT_ERROR, SourceLine(0), "RandomWeighted called with tag %d has all zero weights", randomTag);
            n--;
        }
        return n - 1;
    }
...
const void *RandomElementArray(u32 randomTag, const void *array, u32 size, u32 count)
{
    ...
    if (turn && turn->rng.tag == randomTag)
    {
        ...
        if (index == count)
        {
            ...
            Test_ExitWithResult(TEST_RESULT_ERROR, SourceLine(0), "%s: RandomElement illegal value requested: %d", filename, turn->rng.value);
        }
    }
    ...
    if (randomTag == STATE->rngTag && CanCheckRngTag())
    {
        ...
        {
            Test_ExitWithResult(TEST_RESULT_ERROR, SourceLine(0), "RandomElement called with inconsistent trials %d and %d", STATE->trials, count);
        }
...
static s32 TryAbilityPopUp(s32 i, s32 n, u32 battlerId, u32 ability)
{
    ...
    for (; i < iMax; i++)
    {
        ...
        if (event->battlerId == battlerId && (event->ability == ABILITY_NONE || event->ability == ability))
            return i;
    }
    return -1;
}
...
static s32 TryAnimation(s32 i, s32 n, u32 animType, u32 animId)
{
    ...
    for (; i < iMax; i++)
    {
        ...
        if (event->type == animType && event->id == animId
...
{
    return DATA.chosenGimmick[(battlerId & BIT_SIDE)][partyIndex];
}
...
void TestRunner_Battle_AfterLastTurn(void)
{
    ...
    {
        const char *filename = gTestRunnerState.test->filename;
        Test_ExitWithResult(TEST_RESULT_FAIL, SourceLine(0), "%s:%d: %d TURNs specified, but %d ran", filename, SourceLine(0), DATA.turns, DATA.lastActionTurn + 1);
    }

    while (DATA.queuedEvent < DATA.queuedEventsCount && DATA.queuedEvents[DATA.queuedEvent].groupType == QUEUE_GROUP_NONE_OF)
    {
...
static void CB2_BattleTest_NextTrial(void)
{
    ...
    if (++STATE->runTrial < STATE->trials)
    {
        ...
        DATA.recordedBattle.rngSeed = ISO_RANDOMIZE(STATE->runTrial);
        DATA.queuedEvent = 0;
    ...
    else
    {
        ...
        if (abs(STATE->observedRatio - STATE->expectedRatio) <= Q_4_12(0.02))
            ...
            Test_ExitWithResult(TEST_RESULT_FAIL, SourceLine(0), "Expected %q passes/successes, observed %q", STATE->expectedRatio, STATE->observedRatio);
    }
}
...
{
    bool32 madeProgress = STATE->checkProgressParameter < STATE->runParameter
                       || STATE->checkProgressTrial < STATE->runTrial
                       || STATE->checkProgressTurn < gBattleStruct->battleTurnCounter;
...
{
    if (result != TEST_RESULT_INVALID && result != TEST_RESULT_ERROR && result != TEST_RESULT_TIMEOUT && STATE->runTrial < STATE->trials)
    {
...
static void CreateTestMon(u32 species, u32 gender)
{
    ...
    CreateMon(DATA.currentMon, generator);
...
void ClosePokemon(u32 sourceLine)
{
    ...
    for (i = 0; i < STATE->battlersCount; i++)
    {
        if ((i & BIT_SIDE) == DATA.currentSide && DATA.currentMonIndexes[i] == DATA.currentPartyIndex)
        {
...
void Ability_(u32 sourceLine, u32 ability)
{
    ...
    if (info->hiddenAbility == ability)
    {
        isHidden = TRUE;
        SetMonData(DATA.currentMon, MON_DATA_ABILITY_HIDDEN, &isHidden);
        return;
    }
    ...
    INVALID_IF(i == 2, "%S cannot have %S", gSpeciesInfo[species].name, gAbilities[ability].name);
}
...
void Status1_(u32 sourceLine, u32 status1)
{
    ...
    SetMonData(DATA.currentMon, MON_DATA_STATUS_ID, &status1);
}
...
void GigantamaxFactor_(u32 sourceLine, bool32 factor)
{
    ...
    SetMonData(DATA.currentMon, MON_DATA_GIGANTAMAX_FACTOR, &factor);
}

void DynamaxLevel_(u32 sourceLine, u32 level)
{
    INVALID_IF(!DATA.currentMon, "DynamaxLevel outside of PLAYER/OPPONENT");
    INVALID_IF(level > MAX_DYNAMAX_LEVEL, "Illegal dynamax level: %d", level);
    SetMonData(DATA.currentMon, MON_DATA_DYNAMAX_LEVEL, &level);
}
...
void BattleTest_CheckBattleRecordActionType(u32 battlerId, u32 recordIndex, u32 actionType)
{
    ...
    if (actionType == RECORDED_MOVE_SLOT && recordIndex > 0 && DATA.battleRecordTypes[battlerId][recordIndex - 1] != RECORDED_ACTION_TYPE)
    {
        s32 i;
        const char *filename = gTestRunnerState.test->filename;
        ...
        {
            if (DATA.battleRecordTypes[battlerId][i - 1] == RECORDED_ACTION_TYPE && DATA.recordedBattle.battleRecord[battlerId][i - 1] == B_ACTION_USE_MOVE)
            {
    ...
    {
        DATA.lastActionTurn = gBattleStruct->battleTurnCounter;
    ...
    {
        if (DATA.lastActionTurn == gBattleStruct->battleTurnCounter)
        {
            const char *filename = gTestRunnerState.test->filename;
            Test_ExitWithResult(TEST_RESULT_FAIL, SourceLine(0), "%s:%d: TURN %d incomplete", filename, SourceLine(0), gBattleStruct->battleTurnCounter + 1);
        }
    }
}
...
void CloseTurn(u32 sourceLine)
{
    ...
    for (i = 0; i < STATE->battlersCount; i++)
    {
        if (!(DATA.actionBattlers & (1 << i)))
            Move(sourceLine, &gBattleMons[i], (struct MoveContext) { move: TEST_DEFAULT_MOVE, explicitMove: TRUE });
    }
...
void Move(u32 sourceLine, struct BattlePokemon *battler, struct MoveContext ctx)
{
    ...
    if (ctx.explicitMove)
    {
        ...
        for (i = 0; i < MAX_MON_MOVES; i++)
        {
            ...
            {
                INVALID_IF(DATA.explicitMoves[battlerId & BIT_SIDE] & (1 << DATA.currentMonIndexes[battlerId]), "Missing explicit %S", gBattleMoves[ctx.move].name);
    ...
    else
    {
        ...
        if (move->target == MOVE_TARGET_RANDOM || move->target == MOVE_TARGET_BOTH || move->target == MOVE_TARGET_FOES_AND_ALLY || move->target == MOVE_TARGET_OPPONENTS_FIELD
        || move->target == MOVE_TARGET_OPPONENTS || move->target == MOVE_TARGET_DEPENDS)
        {
            target = BATTLE_OPPOSITE(battlerId);
        }
        else if (move->target == MOVE_TARGET_SELECTED)
        {
            INVALID_IF(STATE->battlersCount > 2, "%S requires explicit target", gBattleMoves[moveId].name);
        ...
        {
            INVALID("%S requires explicit target", gBattleMoves[moveId].name);
        }
    }
...
void SendOut(u32 sourceLine, struct BattlePokemon *battler, u32 partyIndex)
{
    ...
    if (!(DATA.actionBattlers & (1 << battlerId)))
        Move(sourceLine, battler, (struct MoveContext) { move: TEST_DEFAULT_MOVE, explicitMove: TRUE });
