...
void BattleTest_CheckBattleRecordActionType(u32 battlerId, u32 recordIndex, u32 actionType)
{
    ...
    if (DATA.nextRNGTurn == gBattleStruct->battleTurnCounter && (DATA.recordedBattle.rngSeed == RNG_SEED_DEFAULT
    || DATA.recordedBattle.rngSeed == RNG_SEED_CRITICAL_HIT || DATA.recordedBattle.rngSeed == RNG_SEED_MISS))
    {
...
void Move(u32 sourceLine, struct BattlePokemon *battler, struct MoveContext ctx)
{
    ...
    // advance.
    if (gBattleMoves[moveId].accuracy != 0 || gBattleMoves[moveId].split != SPLIT_STATUS)
    {
